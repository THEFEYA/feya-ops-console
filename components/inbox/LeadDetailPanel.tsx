  ...
  async function handleActionStatus(actionId: string, status: 'done' | 'canceled') {
    try {
      ...
      if (!res.ok) {
        ...
        setActions((prev) => prev.map((a) => a.action_id === actionId ? json.data : a))
        toast.success(status === 'done' ? 'Отмечено выполненным' : 'Отменено')
      }
    } catch {
    ...
    <aside className="w-[400px] flex-shrink-0 glass-card border-l border-border flex flex-col overflow-hidden animate-slide-in-right">
      ...
      <div className="p-4 border-t border-border space-y-3">
        ...
        <div>
          ...
          {/* Note + due date for the next quick action */}
          <div className="space-y-1.5">
            <input
              type="text"
              value={actionNote}
